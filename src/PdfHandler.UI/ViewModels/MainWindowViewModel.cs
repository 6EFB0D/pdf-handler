...
{
    /// <summary>初期化完了時に発火（スプラッシュ画面を閉じるために使用）</summary>
    ...
    [ObservableProperty]
    private double _thumbnailLoadProgress = 0;
    ...
    private async Task InitializeAsync()
    {
        ...
        {
            // 正常完了・エラー問わず、スプラッシュを閉じるためにイベントを発火
    ...
    // フォルダ読み込み中キャンセル用トークン（フォルダ切替時に前の読み込みを中断）
    private System.Threading.CancellationTokenSource? _loadFolderCts;

    private async Task LoadPdfFilesAsync(string folderPath)
    {
        // 前回の読み込みをキャンセル（Disposeはしない。古いタスクがまだ token を参照中の場合があるため GC に任せる）
        _loadFolderCts?.Cancel();
        ...
        try
        {
            ...
            if (total > 0)
            {
                ...
                foreach (var file in files)
                {
                    ...
                    ThumbnailLoadProgress = (double)done / total * 100;
                }
