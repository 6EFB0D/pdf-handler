...
- **ボリュームライセンス等**: メールまたはアプリ内のお問い合わせ経路よりご連絡ください（詳細資料は開発用リポジトリで管理）。
...
運用ドキュメント（Supabase／Stripe／内部チェックリスト等）は開発用クローン側にあります。このリポジトリにはソースとユーザー向け資料のほか、[Supabase Edge Functions](supabase/functions/) の参照実装のみを載せています。
