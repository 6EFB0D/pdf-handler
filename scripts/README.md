...
PROD anon key はリポジトリに保存せず、`scripts\Secrets.local.ps1`（Git 管理外）または環境変数/引数で渡してください。
...
```powershell
.\build-release.ps1             # 対話で DEV/PROD を選択
.\build-release.ps1 -TargetEnvironment PROD
.\build-release.ps1 -TargetEnvironment DEV
```
...
#### `build-release.ps1`
DEV / PROD 向けの配布フォルダを、環境ごとに分けて作成します。

事前準備（推奨）:

```powershell
Copy-Item .\Secrets.local.ps1.example .\Secrets.local.ps1
# Secrets.local.ps1 の PDFHANDLER_PROD_SUPABASE_ANON_KEY を実値に置換
```

`Secrets.local.ps1` は `.gitignore` で除外されるため、ローカル固定値として安全に使えます。

引数なしで実行すると、`DEV` / `PROD` を対話で選択できます。
...
出力先:
- PROD: `artifacts\release\prod\PdfHandler-<version>-win-x64\`
- DEV: `artifacts\release\dev\PdfHandler-<version>-win-x64\`

詳細は `docs/release-artifact-layout.md` を参照してください。
