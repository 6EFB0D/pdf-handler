...
| **app_id** | `PDFH` 固定 |
| **Supabase** | 1プロジェクトで複数アプリのライセンスを統合管理可（[multi_app_license_unified_supabase.md](../specs/multi_app_license_unified_supabase.md)） |
| **Supabase スキーマ** | `licenses` に `app_id` カラムを追加し、複数アプリ対応の基盤とする |
| **価格** | ¥4,980（消費税不課税）。不課税であることを明示 |
...
※ Supabase 複数アプリ統合: [multi_app_license_unified_supabase.md](../specs/multi_app_license_unified_supabase.md) 参照
```
...
- [multi_app_license_unified_supabase.md](../specs/multi_app_license_unified_supabase.md)（複数アプリ統合運用時）

### 3.2 実装タスク（概要）

| 項目 | 内容 |
|------|------|
| **Supabase** | `licenses` に `app_id` カラム追加（複数アプリ統合の基盤）。PDF Handler は `app_id='PDFH'` |
...
| **stripe-webhook** | 新形式ライセンスキーの発行ロジック（HMAC署名付き）。`metadata.app_id` で複数アプリ対応 |
| **verify-license** | オンライン検証 + キー正規化（4桁区切り入力対応） |
| **アプリ側** | 5秒タイムアウト、7日間キャッシュ、オフライン時の HMAC 検証 |
| **環境変数** | `LICENSE_SECRET_KEY` の追加 |

### 3.3 複数アプリ統合運用（任意）

Supabase 無料版（プロジェクト2つまで）の制約を避けるため、1プロジェクトで複数アプリを管理する場合:

- **参照**: [multi_app_license_unified_supabase.md](../specs/multi_app_license_unified_supabase.md)
- Stripe Checkout の metadata に `app_id` を含める
- Resend は app_id でテンプレート・送信元を切り替え
...
- **Pro/Team/Enterprise**: Dashboard → **Database** → **Backups** で手動バックアップまたは日次バックアップを確認
- **Free プラン**: ダッシュボードのバックアップ復元は利用不可。事前に CLI でエクスポート推奨:
  ```powershell
  supabase db dump -f backup-$(Get-Date -Format 'yyyyMMdd').sql
  ```
...
### 5.4 スキーマ変更（app_id 追加）

複数アプリ統合運用に対応するため、`licenses` に `app_id` を追加します。

```sql
ALTER TABLE licenses ADD COLUMN IF NOT EXISTS app_id TEXT NOT NULL DEFAULT 'PDFH';

-- 既存レコードの app_id を PDFH で統一（デフォルトで設定済みの場合は不要）
...
| 変数 | 新しい値 |
|------|----------|
| `STRIPE_PRICE_ID_PURCHASED` | 新しい Price ID（必須） |
...
- アプリ内の料金表示（`LicenseDialog.xaml` など）※ ¥4,980（消費税不課税）で統一
- `docs/specs/license-code-specification.md`（HMAC 新形式の反映）
- `docs/supabase-setup/04_setup-instructions.md` の Stripe 価格・app_id 記載
- `docs/specs/multi_app_license_unified_supabase.md` の参照（複数アプリ統合時）
...
- [x] アプリ側: 5秒タイムアウト・7日キャッシュ・HMACオフライン検証を実装
- [x] アプリ側: ライセンスキー4桁区切り表示・正規化
- [x] stripe-webhook: HMAC署名付きキー発行ロジックを実装
- [x] stripe-webhook: `metadata.app_id` 対応・複数アプリ統合
- [x] create-checkout-session: `metadata.app_id` を Stripe に渡す
- [x] verify-license: キー正規化（4桁区切り入力対応・任意 app_id 対応）
- [x] `10_add_app_id.sql` マイグレーション作成
- [ ] Supabase SQL Editor で `docs/supabase-setup/10_add_app_id.sql` を実行
- [ ] Supabase Secrets に `LICENSE_SECRET_KEY` を設定（stripe-webhook・アプリ環境変数用）

### 消費税の適正化（Phase 4）
- [ ] Stripe で買い切り Price を ¥4,980（消費税不課税）に変更・設定
- [x] アプリ内の価格表示を「消費税不課税」に統一（LicenseDialog / PurchaseDialog）
- [x] USER_MANUAL に消費税取り扱いを追記
- [ ] 領収証・請求書の表示ルールを整備（運用時）

### Supabase（Phase 5）
- [ ] バックアップを取得（Pro 以上: Dashboard / Free: `supabase db dump`）
- [ ] レコードクリーンアップ: `docs/supabase-setup/11_cleanup_records.sql` を SQL Editor で実行
- [x] スキーマ変更（app_id）: `10_add_app_id.sql` で対応済み
...
- [複数アプリ統合ライセンス管理](../specs/multi_app_license_unified_supabase.md)（1 Supabase プロジェクトで複数アプリ）