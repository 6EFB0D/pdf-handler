# 運用ガイド（購入・ライセンス・サポート）

購入・ライセンス発行・サポートまわりの運用ドキュメントです。

## ドキュメント一覧

| 番号 | 内容 | ファイル |
|------|------|----------|
| 1 | ライセンス発行後のメール自動配信 | [01_license-email-delivery.md](01_license-email-delivery.md) |
| 2 | 返金を求められた際の処理方法 | [02_refund-processing.md](02_refund-processing.md) |
| 3 | 領収証の発行（企業向け・個人情報保護） | [03_receipt-issuance.md](03_receipt-issuance.md) |

## 個人情報を公開しないための共通ポイント

- **連絡先**: 個人メールではなく `support@yourdomain.com` などの事業者用メールを使用
- **領収証・領収書**: Stripe の自動領収証を活用し、発行者は事業者名のみ
- **問い合わせ**: フォーム経由で受け、送信先を support@ に転送
- **メール送信**: Resend 等で `noreply@` や `support@` から送信
