...
`pdf-handler/supabase/functions/` 直下に**11 関数**があり、**DEV / PROD ともに同一ソース**をデプロイする（差は `--project-ref` と Secrets のみ）。  
新規発行ライセンスキーは **コンパクト形式**（記号としてハイフン除く 32 文字・DB には 4 文字ごとにハイフンを入れた正準形で保存）。`_shared/compact-license-key.ts` を参照する関数は CLI がバンドル時に同梱する。
...
### 3-A-2. DEV へのデプロイ（PROD と同一実装）

**License Manager_DEV** にも、上記と**同じ 11 本**をデプロイしておくと、コンパクト鍵・`verify-license` の正規化・管理系 API が PROD と同じ挙動になる。

1. `pdf-handler` ルートで `npx supabase link --project-ref <DEV_PROJECT_REF>`（既に link 済みなら省略可）
2. §3-A の `npx supabase functions deploy ...` を **`<PROD_PROJECT_REF>` を `<DEV_PROJECT_REF>` に置き換えて**再実行

文書上の DEV 例: `yzmjuotvkxcfnsgleyxl`（実際の ID は Dashboard → Project Settings → General の Reference ID で確認）。

```powershell
cd D:\Users\admin_mak\project\pdf-handler
# §3-A の deploy 11 行をそのままコピーし、<PROD_PROJECT_REF> を <DEV_PROJECT_REF> に置換して実行
# 文書例の DEV ref: yzmjuotvkxcfnsgleyxl（Dashboard の Reference ID と要照合）
```

> **Secrets**: DEV でも `LICENSE_SECRET_KEY` 等は **PROD と別値**（§3-B 参照）。両方に同じキーを置かない。
...
| 日付 | 内容 |
|------|------|
| 2026-04-27 | §3-A にコンパクト鍵・`_shared` を明記。§3-A-2 で DEV へ PROD と同一 Edge をデプロイする手順を追加。`docs/.../03_edge-functions` に README・スナップショット同期の扱いを整理。 |
