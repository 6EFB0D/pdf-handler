...
## 9. GitHub Releases の配布物（v1.1.3 以降）

ビルド手順: `.\scripts\build-release.ps1 -TargetEnvironment PROD` のあと `.\tools\build-release.ps1 -TargetEnvironment PROD`

### Assets に必ず含めるもの（手動アップロード）

- [ ] `PdfHandler-<version>-prod-setup.exe`
- [ ] `PdfHandler-<version>-prod-setup-checksum.txt`
- [ ] `PdfHandler-<version>-prod-setup.zip`（**setup.exe を ZIP にしたもの**。v1.1.2 と同じ）

GitHub 自動生成の **Source code (zip/tar.gz)** はソース用であり、インストーラではない。

### GitHub に載せないもの

- [ ] `PdfHandler-<version>-win-x64.zip`（展開型・**公開しない**）

### Release 本文に書くこと（要約）

- setup.exe をダウンロードしてインストール（ブロック時は setup.zip）
- 製品ページ: https://6efb0d.github.io/office-goplan/pdfhandler.html
- お問い合わせ: Google フォーム

詳細な成果物パスは [`release-artifact-layout.md`](./release-artifact-layout.md) を参照。
